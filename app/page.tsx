...
const nav: { label: View; icon: string }[] = [
  { label: "Today", icon: "âŒ‚" }, { label: "Plan", icon: "â–¦" },
  { label: "Coach", icon: "âœ¦" }, { label: "Connections", icon: "â†—" },
];
...
function Source({ mark, name, detail }: { mark: string; name: string; detail: string }) {
  return <button className="source-row"><span className={`source-mark ${name.toLowerCase().split(" ")[0]}`}>{mark}</span><span><b>{name}</b><small>{detail}</small></span><i>Connected</i><em>â€º</em></button>;
}
  ...
  return <>
    <section className="hero-grid">
      <article className="card workout-card">
        <div className="eyebrow"><span>Todayâ€™s workout</span><span className="pill">Key session</span></div>
        <h2>Progression run <span>Â· 50 min</span></h2>
        <p className="workout-note">Build smoothly from conversational to strong. Youâ€™re well recovered, so the final 10 minutes can be purposeful.</p>
        ...
        <button className="primary" onClick={onStart}>â–¶ &nbsp; Start workout</button>
      </article>
      <article className="card readiness-card">
        <div className="card-head"><h3>Readiness</h3><button aria-label="Readiness information">â“˜</button></div>
      ...
      <article className="card environment-card">
        ...
        <div className="weather-row"><span className="weather-icon">â˜€</span><div><strong>6:30â€“8:00 AM</strong><small>Best outdoor conditions</small></div></div>
        <div className="condition-grid"><span><b>64Â°</b><small>Temperature</small></span><span><b>34</b><small>AQI Â· Good</small></span><span><b>6 mph</b><small>Wind</small></span><span><b>42%</b><small>Humidity</small></span></div>
        ...
        <button className="text-button">View hourly outlook <span>â†’</span></button>
      </article>
    </section>
    <section className="lower-grid">
      <article className="card load-card"><div className="card-head"><div><h3>Weekly load</h3><p>Right where you want to be</p></div><b className="load-number">412 <small>/ 450â€“550</small></b></div><div className="chart-bars">{[38,58,47,66,82,44,73].map((v,i)=><div key={i}><span style={{height:`${v}%`}}></span><small>{["M","T","W","T","F","S","S"][i]}</small></div>)}</div><div className="chart-footer"><span><i></i> Current load</span><span><i></i> Target range</span><b>â†‘ 8% vs last week</b></div></article>
      <article className="card race-card"><div className="eyebrow">Goal race</div><h3>Santa Cruz Half</h3><div className="race-center"><Ring value={68} label="Plan complete" small /><div><strong>9 weeks</strong><span>September 20, 2026</span></div></div><div className="race-stats"><span><b>1:38:00</b><small>Goal</small></span><span><b>7:29/mi</b><small>Goal pace</small></span></div><button className="text-button">Open race plan <span>â†’</span></button></article>
      <article className="card recovery-card"><div className="card-head"><h3>Recovery insight</h3><span>â˜¾</span></div><h4>Strong recovery detected.</h4><p>Your body is responding well. Add 10â€“15 minutes of mobility after todayâ€™s run.</p><div className="signals"><span><b>Low</b> muscle soreness</span><span><b>Good</b> energy</span><span><b>Stable</b> sleep trend</span></div></article>
    </section>
  </>;
}
...
function CoachView() {
  ...
  return <section className="feature-layout coach-layout"><article className="card coach-panel"><div className="coach-avatar">âœ¦</div><div><div className="eyebrow">RunFormance coach</div><h2>Training that adjusts with you.</h2><p>I combine your recent load, recovery, schedule, race goal, and local conditionsâ€”then explain the recommendation.</p></div><div className="suggestions"><button onClick={()=>setMessage("Can I swap todayâ€™s run for cycling?")}>Swap todayâ€™s workout</button><button onClick={()=>setMessage("Why is my readiness 82?")}>Explain my readiness</button><button onClick={()=>setMessage("Adjust my week around travel")}>Plan around travel</button></div><div className="chat-history">{chat.map((x,i)=><div key={i} className={i%2?"bot":"user"}>{x}</div>)}</div><form onSubmit={e=>{e.preventDefault(); if(message){setChat([...chat,message,"Yes. A 50-minute Zone 2 ride preserves the aerobic goal with less impact. Iâ€™ll move the progression run to Friday, when recovery and air quality both look favorable."]);setMessage("")}}}><input value={message} onChange={e=>setMessage(e.target.value)} placeholder="Ask about training, recovery, or your planâ€¦"/><button>Send</button></form></article><article className="card principles"><h3>Todayâ€™s decision</h3><div className="decision-score"><Ring value={82} label="Go" small/></div><h4>Why this workout?</h4><ul><li>Acute load is within your optimal range</li><li>HRV is 9% above your 30-day baseline</li><li>You had 48 hours since your last hard effort</li><li>Morning AQI supports outdoor training</li></ul><p className="fine-print">Recommendations are educational and do not replace medical advice.</p></article></section>;
}

function ConnectionsView() {
  const [location,setLocation]=useState(true); const [alerts,setAlerts]=useState(true);
  return <section className="feature-layout"><article className="card connection-list"><div className="eyebrow">Your data</div><h2>One view of your whole training life.</h2><p>Connect the services you choose. You control what is shared and can disconnect at any time.</p><Source mark="â™¥" name="Apple Health" detail="Sleep, HRV, workouts Â· synced 2 min ago"/><Source mark="G" name="Garmin Connect" detail="Training load, recovery, activities Â· synced 3 min ago"/><Source mark="â–²" name="Strava" detail="Activities, routes, segments Â· synced 2 min ago"/><button className="add-source">ï¼‹ Add another source</button></article><article className="card privacy-card"><div className="card-head"><h3>Location & conditions</h3><span>Private by design</span></div><p>Use approximate location for hyperlocal workout guidance. Precise workout routes are never required.</p><label className="toggle-row"><span><b>Local conditions</b><small>Weather, AQI, smoke, heat, UV and pollen</small></span><input type="checkbox" checked={location} onChange={e=>setLocation(e.target.checked)}/><i></i></label><label className="toggle-row"><span><b>Rapid-change alerts</b><small>Notify me when conditions change before a workout</small></span><input type="checkbox" checked={alerts} onChange={e=>setAlerts(e.target.checked)}/><i></i></label><div className="location-preview"><b>Yuba City, California</b><span>Approximate location Â· updated 12 min ago</span><button>Change</button></div><div className="privacy-note">Your health and location data is used only to personalize recommendations. It is never sold.</div></article></section>;
}
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    {modal&&<div className="modal-backdrop" onMouseDown={()=>setModal(false)}><div className="modal" onMouseDown={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setModal(false)}>Ã—</button><div className="coach-avatar">â–¶</div><div className="eyebrow">Ready when you are</div><h2>Progression run Â· 50 min</h2><p>Weâ€™ll guide each stage with pace and effort cues. Your watch will receive the workout automatically.</p><div className="modal-stats"><span><b>82</b> readiness</span><span><b>34</b> AQI</span><span><b>64Â°F</b> outside</span></div><button className="primary" onClick={()=>setModal(false)}>Send to Garmin & start</button><button className="secondary" onClick={()=>setModal(false)}>Start without a device</button></div></div>}
  </div>;
}
